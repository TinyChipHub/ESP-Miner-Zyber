...
esp_err_t ASIC_set_device_model(GlobalState * GLOBAL_STATE)
{
    ...
    if (strcmp(GLOBAL_STATE->device_model_str, "zyber8s") == 0) {
        ...
        GLOBAL_STATE->chips_count = ZYBER8S_ASIC_COUNT;
        // GLOBAL_STATE.asic_job_frequency_ms = (NONCE_SPACE / (double) (GLOBAL_STATE.POWER_MANAGEMENT_MODULE.frequency_value *
        // BM1368_CORE_COUNT * 1000)) / (double) BITAXE_SUPRA_ASIC_COUNT; // version-rolling so Small Cores have different Nonce
        // Space
        ...
        // GLOBAL_STATE.asic_job_frequency_ms = (NONCE_SPACE / (double) (GLOBAL_STATE.POWER_MANAGEMENT_MODULE.frequency_value *
        // BM1370_CORE_COUNT * 1000)) / (double) BITAXE_GAMMA_ASIC_COUNT; // version-rolling so Small Cores have different Nonce
        // Space
        GLOBAL_STATE->asic_job_frequency_ms = 500; // ms
        ...
        GLOBAL_STATE->asic_job_frequency_ms = 500; // ms
        GLOBAL_STATE->ASIC_difficulty = BM1370_ASIC_DIFFICULTY;
        ESP_LOGI(TAG, "DEVICE: zyber8gplus");
