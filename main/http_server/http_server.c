...
static esp_err_t PATCH_update_settings(httpd_req_t * req)
{
    ...
    if ((item = cJSON_GetObjectItem(root, "beneathfanspeed")) != NULL) {
        nvs_config_set_u16(NVS_CONFIG_BENEATH_FAN_SPEED, item->valueint);
    }
...
static esp_err_t GET_system_info(httpd_req_t * req)
{
    ...
    cJSON_AddNumberToObject(root, "beneathfanspeed", GLOBAL_STATE->POWER_MANAGEMENT_MODULE.beneath_fan_perc);
    cJSON_AddNumberToObject(root, "fanrpm", GLOBAL_STATE->POWER_MANAGEMENT_MODULE.fan_rpm);
    cJSON_AddNumberToObject(root, "runningMode", nvs_config_get_u16(NVS_CONFIG_RUNNING_MODE, 1));
